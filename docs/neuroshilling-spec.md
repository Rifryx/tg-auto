...
3. [Структура данных (БД)](#3-структура-данных)
4. [API-эндпоинты](#4-api-эндпоинты)
5. [Worker / Воркер](#5-worker)
6. [LLM-интеграция](#6-llm-интеграция)
...
└── llm/
    ...
    └── scenario_generator.py # Генерация сценариев (роли + шаги)
```
...
### 3.1 `shilling.campaigns` — Кампания

| Поле | Тип | Описание |
|------|-----|----------|
| `id` | BIGINT PK | |
| `name` | VARCHAR | Название кампании |
| `brand_name` | VARCHAR NULL | Бренд/ключевая фраза для упоминания |
| `brand_link` | VARCHAR NULL | Ссылка бренда (опционально, для контекста LLM) |
| `topic` | TEXT NULL | Тема обсуждения (для генерации сценария ИИ) |
...
### 3.2 `shilling.scenarios` — Сценарий
...
### 3.3 `shilling.scenario_roles` — Роли в сценарии
...
### 3.4 `shilling.scenario_steps` — Шаги диалога (реплики)
...
### 3.5 `shilling.campaign_accounts` — Аккаунты кампании
...
### 3.6 `shilling.campaign_targets` — Целевые каналы
...
### 3.7 `shilling.execution_logs` — Лог выполнения
...
### 3.8 `shilling.blacklist` — Чёрный список
...
### 5.2 Orchestrator — жизненный цикл кампании
...
### 5.3 Failover — замена аккаунта при бане
...
### 6.1 Промпт для рерайтинга (ReplicaRewriter)
...
### 6.2 Промпт для генерации сценария (ScenarioGenerator)
...
### 7.2 Структура файлов фронтенда
...
### 7.3 Экран кампании — структура блоков (сверху вниз)
...
- **Режим «Оживление чата»** — отложено, только режим Кампания (продвижение)
